...
RSpec.describe Cenabast::Api::ProviderInformationFetcher do
  describe '#call - valid run' do
    before(:each) do
      VCR.insert_cassette 'cenabast/api/provider_information_valid', erb: true
    end
  ...
  describe '#call - invalid run' do
    before(:each) do
      VCR.insert_cassette 'cenabast/api/provider_information_invalid', erb: true
    end
